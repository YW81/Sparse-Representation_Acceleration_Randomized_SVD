function [X, accuracy, avgTime] = svd_fsc_main(Y, A, D, L, sc_algo, trainLabel, testLabel, numClass, displayProgressFlag,k)
...
for i = 1: size(Q,2)
    ...
    [WA_reduced, releventPosition] = reduceMatrix(q, WA);
    
    % sparse coding: solve a reduced linear system
    tic
    tpReduced = sparse_coding_methods(xInit(releventPosition), WA_reduced, q, sc_algo);
    t = toc;

    sumTime = sumTime+t;

    tp(releventPosition)=tpReduced;
